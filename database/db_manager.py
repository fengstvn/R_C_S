...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # 插入示例公告数据（仅当表为空时插入，避免每次启动重复插入）
            cursor.execute('SELECT COUNT(*) FROM notices')
            if cursor.fetchone()[0] == 0:
                cursor.execute('''
                    INSERT INTO notices (title, content, remark)
                    VALUES
    ...
    def update_user_score(self, user_id: int, delta: int, conn=None):
        """更新用户积分

        修复打卡异常：支持传入已有的数据库连接，
        避免在 create_record 的事务中再开新连接导致 database is locked。
        """
        if conn is not None:
            conn.execute(
    ...
    def increment_checkin_count(self, place_id: int, conn=None):
        """增加景点打卡次数
    ...
    def create_record(self, user_id: int, place_id: int, experience: str = '') -> int:
        ...
        修复打卡异常：原先在事务中调用 update_user_score /
        increment_checkin_count，这两个方法会各自新开数据库连接，
        外层连接持有写锁导致内层报 "database is locked"，
        打卡必然失败。现在全部操作复用同一个连接，一次事务提交。
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            ...
            # 用户积分+1（复用同一连接）
            self.update_user_score(user_id, 1, conn=conn)
            # 景点打卡次数+1（复用同一连接）
            self.increment_checkin_count(place_id, conn=conn)
    ...
    def delete_record(self, record_id: int):
        ...
        修复：与 create_record 同理，积分扣减复用同一连接，
        避免嵌套连接导致 database is locked。
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
